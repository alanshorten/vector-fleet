...
IMPORTANT - do not fabricate a removal from narrative text. Documents sometimes include a closing narrative sentence about a ferry flight, redelivery, or lease-end event (e.g. "Aircraft MSN 1336 ferried to ... with engine 779804 installed on position 1. Post ferry flight engine TSN ... CSN ..."). Read this kind of sentence carefully: if it says the engine WAS installed / remained installed during that flight, that is NOT a removal - it is a status update confirming the engine is still on the aircraft, usually with a newer TSN/CSN reading than the table's last row. In that case do NOT create a new row for it; instead fold the newer TSN/CSN reading into the existing open stint (the one with no removal date) as its tsnAtRemoval/csnAtRemoval (representing "current" reading, per that field's definition below), leave that stint's removalDate blank, and let asOfDate reflect the date in that sentence if it's the latest date in the document. Only ever create a removal row when the document actually states the engine came OFF an aircraft - a redelivery/ferry-flight sentence describing the engine as installed is the opposite of that, and stints must never be invented from text that doesn't describe an install or removal.
