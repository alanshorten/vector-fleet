...
function GuideView({userRole}){
  ...
  const sections=[
    ...
    {title:"Adding & Managing Assets",icon:"✈",content:[
      ...
      ["Create via Email","Send a utilisation report PDF to your organisation's email-ingestion address (ask an administrator). The system extracts and processes it automatically. Low-severity reports apply immediately; high-severity reports (S/N change, delta mismatch, gap) are held in the Dashboard review queue."],
    ...
    {title:"Uploading Reports",icon:"📤",content:[
      ...
      ["Utilisation Report — Email","Send the PDF to your organisation's email-ingestion address (ask an administrator). Processing is automatic — the same AI extraction runs server-side and the result appears in the app within seconds. High-severity warnings route to the Dashboard review queue instead of applying immediately."],
