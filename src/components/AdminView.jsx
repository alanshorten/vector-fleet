...
function AdminView({assets,notify,userRole}){
  const { mode: layoutMode } = useLayoutMode();
  // Tab visibility — Guide + Settings: every role. Knowledge Base: Editor
  // and Admin (not Viewer/Data Entry — matches "Data Entry doesn't see
  // financial outputs" already established for the main nav). Each tab
  // body re-checks its own gate too (defense in depth — same pattern
  // App.jsx already uses for the old Admin view), not just the nav row,
  // in case tab state is ever set some other way.
  //
  // Admin Panel + Platform used to be tabs here too (see TECH_DEBT.md /
  // the 21 Aug delivery docs for that brief detour) — Alan asked for them
  // pulled into their own "Admin" hamburger entry instead, separate from
  // this everyone-sees-it Settings screen. That's AdminPanelView.jsx now,
  // routed from App.jsx as view==="admin".
...
export { AdminView, AirframePhotoSettings, DisclaimerSettings, EnginePhotoSettings, KnowledgeBaseSettings, LogoSettings };