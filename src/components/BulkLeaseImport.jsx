...
function BulkLeaseImport({assets,saveAsset,notify}){
  ...
  const STATUS_META={
    matching:{label:"Reading & matching…",color:"var(--color-graphite)"},
    unsupported:{label:"⚠ Not a PDF or Word file",color:"var(--color-critical)"},
    "needs-manual-entry":{label:"⚠ Needs manual entry — no extractable text (likely scanned)",color:"var(--color-attention)"},
    unmatched:{label:"Needs manual assignment",color:"var(--color-attention)"},
    matched:{label:"Matched — ready to extract",color:"var(--color-positive)"},
    "needs-page":{label:"Confirm the rate schedule section",color:"var(--color-graphite)"},
    extracting:{label:"Extracting…",color:"var(--color-graphite)"},
    ready:{label:"Parsed — review & save",color:"var(--color-teal)"},
    saving:{label:"Saving…",color:"var(--color-graphite)"},
    saved:{label:"✓ Details saved — pots pending",color:"var(--color-attention)"},
    activating:{label:"Activating…",color:"var(--color-graphite)"},
    "activate-review":{label:"⚠ Review flagged rates",color:"var(--color-attention)"},
    activated:{label:"✓ Lease activated",color:"var(--color-positive)"},
    skipped:{label:"Skipped",color:"var(--color-graphite)"}
  };
  ...
  if(!tier){
    return(
      <div className="card" style={{padding:24}}>
        ...
        <div style={{fontSize:11,color:"var(--color-graphite)",fontWeight:700,marginBottom:10}}>CHOOSE HOW TO PARSE THIS BATCH (applies to every file)</div>
        <div style={{display:"flex",flexDirection:"column",gap:10}}>
          <button className="btn btn-ghost" style={{padding:"12px 14px",textAlign:"left"}} onClick={()=>setTier("quick")}>
            ...
            <div style={{fontSize:11,color:"var(--color-graphite)",marginTop:2}}>Each whole document is processed automatically in one pass — most complete, since lessee/dates and rates can live on different pages.</div>
          </button>
          <button className="btn btn-ghost" style={{padding:"12px 14px",textAlign:"left"}} onClick={()=>setTier("confidential")}>
            ...
            <div style={{fontSize:11,color:"var(--color-graphite)",marginTop:2}}>Nothing is sent until you confirm the rate-schedule section for each document — the rest of every lease stays in your browser.</div>
          </button>
        </div>
      </div>
    );
    ...
    <div>
      <div className="card" style={{padding:16,marginBottom:14}}>
        <div className="flj" style={{alignItems:"center"}}>
          <div style={{fontSize:12,color:"var(--color-graphite)"}}>
            Batch mode: <strong style={{color:"var(--color-carbon)"}}>{tier==="quick"?"⚡ Quick Extract":"🔒 Confidential Extract"}</strong> — adding a lease for one asset? Use the 📄 Lease button on that asset's page instead.
          </div>
      ...
      <div className="card" style={{padding:24,textAlign:"center",marginBottom:16,border:"2px dashed var(--color-divider)"}}>
        ...
        <label htmlFor="bulkleasefile" style={{cursor:"pointer"}}>
          <div style={{fontWeight:600,color:"var(--color-graphite)"}}>{queue.length?"Click to add more files":"Click to select lease documents"}</div>
          <div style={{fontSize:12,color:"var(--color-graphite)",marginTop:4}}>Multiple PDF or Word (.docx) files</div>
        </label>
      </div>
      ...
      {queue.length>0&&(
        <>
          <div className="flab g8" style={{marginBottom:14,fontSize:11}}>
            {counts.matched>0&&<span className="pill" style={{background:"var(--color-positive-tint)",color:"var(--color-positive)"}}>{counts.matched} matched</span>}
            {counts.needsAssignment>0&&<span className="pill" style={{background:"var(--color-attention-tint)",color:"var(--color-attention)"}}>{counts.needsAssignment} needs assignment</span>}
            {counts.needsManualEntry>0&&<span className="pill" style={{background:"var(--color-critical-tint)",color:"var(--color-critical)"}}>{counts.needsManualEntry} needs manual entry</span>}
            {counts.ready>0&&<span className="pill" style={{background:"var(--color-teal-tint)",color:"var(--color-teal)"}}>{counts.ready} ready to save</span>}
            {counts.saved>0&&<span className="pill" style={{background:"var(--color-attention-tint)",color:"var(--color-attention)"}}>{counts.saved} saved (pots pending)</span>}
            {counts.activated>0&&<span className="pill" style={{background:"var(--color-positive-tint)",color:"var(--color-positive)"}}>{counts.activated} activated</span>}
          </div>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:10}}>
            {queue.map(row=>{
              const meta=STATUS_META[row.status]||{label:row.status,color:"var(--color-graphite)"};
              return(
                <div key={row.id} className="card" style={{padding:14,opacity:row.status==="skipped"?0.5:1}}>
                  <div className="flj" style={{alignItems:"center",marginBottom:8}}>
                    <div style={{fontSize:12,fontWeight:700,color:"var(--color-carbon)",wordBreak:"break-all"}}>{row.name}</div>
                    <span style={{fontSize:11,color:meta.color,whiteSpace:"nowrap",marginLeft:10}}>{meta.label}</span>
                  </div>

                  {row.status==="unsupported"&&(
                    <div className="flj" style={{alignItems:"center"}}>
                      <div style={{fontSize:11,color:"var(--color-graphite)"}}>Not a PDF or Word (.docx) document.</div>
                  ...
                  {row.status==="needs-manual-entry"&&(
                    <div>
                      <div style={{fontSize:11,color:"var(--color-graphite)",marginBottom:8}}>No extractable text found — likely scanned or image-only. Open the matching asset's own Lease Wizard to enter this one manually.</div>
                  ...
                  {row.status==="skipped"&&(
                    <div className="flj" style={{alignItems:"center"}}>
                      <div style={{fontSize:11,color:"var(--color-graphite)"}}>Skipped — not imported.</div>
                  ...
                  {(row.status==="matched"||row.status==="extracting")&&(
                    <div>
                      <div className="flj" style={{alignItems:"center",gap:8,marginBottom:row.status==="matched"?8:0}}>
                        <div style={{fontSize:12,color:"var(--color-graphite)"}}>
                          Matched: <strong style={{color:"var(--color-carbon)"}}>MSN {row.matchedAsset.msn} — {row.matchedAsset.registration||"—"}</strong>
                          {row.matchMethod==="auto"&&<span style={{fontSize:10,color:"var(--color-graphite)"}}> (auto)</span>}
                        </div>
                        {row.status==="matched"&&(
                      ...
                      {row.status==="extracting"&&<div style={{fontSize:11,color:"var(--color-graphite)",marginTop:6}}>Extracting…</div>}
                      {row.error&&<div style={{fontSize:11,color:"var(--color-critical)",marginTop:6}}>{row.error}</div>}
                    </div>
                  )}

                  {row.status==="needs-page"&&(
                    <div>
                      <div style={{fontSize:11,color:"var(--color-graphite)",marginBottom:8}}>Confirm which page/section has the reserve rate schedule for MSN {row.matchedAsset.msn}. Only that part is sent — the rest stays in your browser.</div>
                      {row.candidates.filter(c=>c.score>0).length===0&&(
                        <div style={{fontSize:11,color:"var(--color-attention)",marginBottom:8}}>Nothing stood out automatically — showing everything below.</div>
                      )}
                      <div style={{display:"flex",flexDirection:"column",gap:6,maxHeight:220,overflow:"auto"}}>
                        {(row.candidates.filter(c=>c.score>0).length?row.candidates.filter(c=>c.score>0):row.candidates).map(c=>(
                          <button key={c.id} className="btn btn-ghost" style={{padding:"8px 10px",textAlign:"left"}} onClick={()=>runExtractConfidential(row,c.id)}>
                            <div style={{fontWeight:700,fontSize:11,color:"var(--color-carbon)"}}>{c.label}</div>
                            <div style={{fontSize:10,color:"var(--color-graphite)",marginTop:2}}>{c.snippet||"(no extractable text found here)"}</div>
                          </button>
                        ))}
                      </div>
                      {row.error&&<div style={{fontSize:11,color:"var(--color-critical)",marginTop:8}}>{row.error}</div>}
                  ...
                  {row.status==="ready"&&(
                    <div>
                      <div style={{fontSize:11,color:"var(--color-graphite)",marginBottom:6}}>MSN {row.matchedAsset.msn} — {row.matchedAsset.registration||"—"}. Review before saving.</div>
                      ...
                      <div style={{fontSize:10,color:"var(--color-graphite)",fontWeight:700,marginBottom:4}}>RESERVE RATES</div>
                      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6,marginBottom:8}}>
                        {BULK_POT_FIELDS.map(([code,label])=>(
                          <div key={code} className="flj" style={{alignItems:"center",background:"var(--color-divider-inner)",borderRadius:6,padding:"6px 8px"}}>
                      ...
                      {row.parsed?.notes&&<div style={{fontSize:11,color:"var(--color-attention)",marginBottom:8}}>ℹ {row.parsed.notes}</div>}
                      {row.error&&<div style={{fontSize:11,color:"var(--color-critical)",marginBottom:8}}>{row.error}</div>}
                      <div style={{fontSize:10,color:"var(--color-graphite)",marginBottom:8}}>Opening balances aren't captured here — either path leaves them for this asset's own Lease Wizard.</div>
                  ...
                  {row.status==="activating"&&<div style={{fontSize:11,color:"var(--color-graphite)"}}>Validating rates and activating…</div>}
                  {row.status==="saving"&&<div style={{fontSize:11,color:"var(--color-graphite)"}}>Saving…</div>}

                  {row.status==="activate-review"&&row.flaggedPots&&(
                    <div>
                      <div style={{fontSize:11,color:"var(--color-graphite)",marginBottom:8}}>These rates were flagged as unusual — review and acknowledge each one before finishing activation. Nothing has been saved yet.</div>
                      <div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:10}}>
                        {Object.entries(row.flaggedPots).map(([code,fp])=>(
                          <div key={code} style={{background:"var(--color-attention-tint)",border:"1px solid var(--color-attention)",borderRadius:6,padding:10}}>
                            <div style={{fontSize:11,fontWeight:700,color:"var(--color-attention)",marginBottom:4}}>{code} — ${fp.pot.accrualRate}{fp.pot.accrualBasis==="per_FC"?"/FC":fp.pot.accrualBasis==="per_FH"?"/FH":""}</div>
                            <div style={{fontSize:11,color:"var(--color-carbon)",marginBottom:6}}>{fp.message}</div>
                            <label style={{display:"flex",alignItems:"center",gap:6,fontSize:11,color:"var(--color-graphite)",cursor:"pointer"}}>
