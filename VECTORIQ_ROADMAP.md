...
| Supabase FM1 | Stockholm — abandoned | 🔴 Decommission |
| Supabase FM2 | Frankfurt — abandoned | 🔴 Decommission |
...
|-------|------|---------|--------|
| Brain 1 | `utilisation.js` | Parse reports, validate deltas, detect anomalies | ⏳ Built (embedded in UI — extraction still open; bigger scope, tangled with Firebase writes + UI state, needs own session) |
...
1. Move Firebase config to Vercel environment variables
2. Move Cloudinary credentials to Vercel environment variables
3. Move Anthropic API key to Vercel environment variables
4. Tighten Firestore security rules (scope by companyId)
5. Decommission abandoned Supabase projects
6. Firebase Auth — email/password + role-based access
...
### Immediate Priority — Security Pass (1 Session)
- [ ] Move Firebase config to Vercel environment variables
- [ ] Move Cloudinary credentials to Vercel environment variables
- [ ] Move Anthropic API key to Vercel environment variables
- [ ] Tighten Firestore security rules
- [ ] Decommission Supabase FM1 and FM2
- [ ] Consolidate hosting to Vercel only
...
- [ ] Extract Brain 1 (`utilisation.js`) to `/calculations` — still open; larger scope (S/N change detection, report-merge logic tangled with Firebase writes + UI state), deserves a dedicated session with testing, not bundled into other work
...
| Security pass — credentials to Vercel env vars | Build session | 🔴 Before financial data |
| Decommission Supabase projects | Alan | 🔴 Quick win |
| Extract Brain 1 (`utilisation.js`) to `/calculations` | Build session | 🟡 Before Layer 2 build — own session, not bundled |
