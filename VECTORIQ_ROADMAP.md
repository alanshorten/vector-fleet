...
| UI Design System rollout (`TAILIQ_UI_DESIGN_SYSTEM.md`, locked 10 Aug 2026) — sweeping every component off the old dark-navy theme onto the new light "technical grey" theme | Build session | ✅ App-wide sweep complete August 2026 — `styles.css`/`Dashboard.jsx`, `Scenarios.jsx`/`FlyForward.jsx`/`LeaseWizard.jsx`, and the full Asset Details screen (`AssetTabs.jsx`, `PhotosAndSpecs.jsx` — every tab: Overview/Specs/Engines/Landing Gear/APU/Avionics/Photos/History/Documents) all done. Tech spec PDF merge (see next row) now also done — see `TECH_DEBT.md` 4.122–4.123 |
| Tech spec PDF / app design system merge — align `techSpecBuilder.js` body pages onto `TAILIQ_UI_DESIGN_SYSTEM.md`, keeping the navy hero as a distinct cover/identity page | Alan + Opus scoping session | ✅ Done August 2026 — Opus scoping session run, `TAILIQ_UI_DESIGN_SYSTEM.md` §6 updated to the narrower hero-only exception, built against `techSpecBuilder.js` + `public/share.html` — see `TECH_DEBT.md` 4.125 |
| Forward Exposure Summary card — lazy/on-demand implementation | — | ✅ Solved differently, August 2026 — rather than reviving the expensive `buildFleetExposure` approach, split "Portfolio Shortfall Summary" into "Lease Shortfall Summary" + "Post-Lease Shortfall Summary," the latter a cheap aggregation of each pot's already-computed `partialFundedNote` — no lazy-load/spinner pattern needed, sidesteps the original crash cause entirely. See `TECH_DEBT.md` 4.126 |
| "Reserve Position" sign-convention rollout — renamed from "Shortfall Band," flipped display to balance-minus-cost (positive/green = surplus, negative/red = shortfall) across every per-pot table carrying the old convention | Build session | ✅ Done August 2026 — `FlyForward.jsx` pot cards, `Scenarios.jsx` per-pot worst-case table, `PortfolioView.jsx` Route Suitability Matcher table (also fixed a pre-existing bug there where surplus pots showed grey instead of green). Deliberately unchanged: summary/headline totals, Fleet Exposure, EOL Position's contractual payment-adjustment figure (different formula). See `TECH_DEBT.md` 4.127 |
