...
| UI Design System rollout (`TAILIQ_UI_DESIGN_SYSTEM.md`, locked 10 Aug 2026) — sweeping every component off the old dark-navy theme onto the new light "technical grey" theme | Build session | 🟡 In progress — `styles.css`/`Dashboard.jsx` (initial re-skin), `Scenarios.jsx`/`FlyForward.jsx`/`LeaseWizard.jsx` (this session) done. Asset Details screen next up per Alan; remaining components swept file-by-file as they come up for review — see `TECH_DEBT.md` 4.122 |
