...
|-------|------|---------|--------|
| Brain 1 | `utilisation.js` | Parse reports, validate deltas, detect anomalies | ✅ Built (embedded in UI — extract) |
| Brain 2 | `llpCalculator.js` | Track remaining life, extrapolate from old disk sheets | ✅ Built (embedded in UI — extract) |
...
- [ ] Fix Excel upload parsing (SheetJS → text → Claude)
- [ ] Add user-friendly error messages (no raw JSON errors)
- [ ] Extract Brain 1 and Brain 2 to `/calculations` folder
...
| Excel upload fix (SheetJS) | Build session | 🔴 Current bug |
| User-friendly error messages | Build session | 🟡 Same session as above |
